...
import styles from './TableDinamicHv.module.css';
...
const TableInputCell = ({
  ...
  useEffect(() => {
    ...
    // (el usuario no ha escrito nada nuevo)
    if (
      prevInitialValueRef.current !== initialValue &&
      ...
      // Actualizar la referencia aunque no sincronicemos el valor
  ...
  return (
    <BiaInput
      ...
      type={getInputType()}
      value={value}
      containerClassName={styles.inputTableHv}
...
const TableTimePickerCell = ({
  ...
  const formatTimeValue = (val: string): string | undefined => {
    ...
    // Si no se puede parsear, retornar hora actual
    return undefined;
  };
...
const TableDinamicHvComponent = ({
  ...
  if (groups !== prevGroupsRef.current) {
    ...
    if (!groups || groups.length === 0) {
      ...
      // Cada field es una columna
      columnsRef.current = firstGroup.fields.map((field, fieldIndex) => ({
        id: field.field_name || `column_${fieldIndex}`,
        header: () => (
          <BiaText
            color='weak'
            token='caption'
        ...
        cell: (info: CellContext<IGroupsHv, unknown>) => {
          // info.row.index es el índice de la fila (grupo)
          const rowIndex = info.row.index;
          const currentGroup = groups[rowIndex];
          // Obtener el field correspondiente a esta columna en esta fila
...
export const TableDinamicHv = TableDinamicHvComponent;
